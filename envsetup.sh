...
function check_product()
{
    ...
    if (echo -n $1 | grep -q -e "^syberia_") ; then
        SYBERIA_BUILD=$(echo -n $1 | sed -e 's/^lineage_//g')
...
function lunch()
{
    ...
    if [ $? -ne 0 ]
    then
        # if we can't find a product, try to grab it off the syberia GitHub
        T=$(gettop)
        cd $T > /dev/null
        vendor/syberia/build/tools/roomservice.py $product
        cd - > /dev/null
        check_product $product
    else
        T=$(gettop)
        cd $T > /dev/null
        vendor/syberia/build/tools/roomservice.py $product true
        cd - > /dev/null
    fi
